    ...
    (function() {
        ...
        for (i = 0; i < 480; i++) {
            divs += '<div style = "position:absolute;width:1px;height:40px;'
                + 'background-color:#0d0; top:0px; left: ' +i+'px;"></div>';
        }
        console.log(divs);
        $drawTarget.append(divs);
        bars = $drawTarget.children();
        setInterval(function() {
            drawGraph( x * 50, 32 - (sinTable[(x * 20) &4095] * 16), 50 - (sinTable[(x*10)&4095]*20));//draw Graph, change the ang,freq and hight of the
        }, 20);
    })();
});